...
ROCONN.phases.buildIntro = function (cond) {
    ...
    const enterFullscreen = {
        ...
        message: `<h2>${ROCONN.studyName}</h2>
        ...
        stimulus: `
            <div class="text-screen">
                ...
                <blockquote>
                    ...
                    <br><br>
                    Everything runs on your own computer. Please keep your eyes on your
                    own screen and do not discuss the report with anyone until the
                    session ends."
                </blockquote>
...
ROCONN.phases.buildRecallTest = function (cond, phaseLabel) {
    ...
    const intro = {
        type: jsPsychHtmlButtonResponse,
        stimulus: `<div class="text-screen">
                    <h2>${isGateImmediate ? 'Comprehension check' : 'Brief follow-up check'}</h2>
...
ROCONN.phases.buildDistractor = function () {
    ...
    const introDistractor = {
        type: jsPsychHtmlButtonResponse,
        stimulus: `<div class="text-screen">
                    <h2>Short break with brief puzzles</h2>
                    <p>Before the next part, you will complete two short, unrelated
                       puzzle tasks and then a brief rest. These run entirely on your
                       own screen.</p>
                   </div>`,
        choices: ['Begin'],
...
ROCONN.phases.buildBisectionBlock = function (cond, pressure, blockIndex, blockOrder) {
    ...
    const probes = [];
    const repsPerProbe = ROCONN.params.repsPerProbe;
    ROCONN.params.probePositions.forEach(pos => {
        for (let r = 0; r < repsPerProbe; r++) {
            probes.push({
                probe_pos: pos,
...
ROCONN.phases.buildPostMeasures = function () {
    ...
    const confidence = {
        type: jsPsychSurveyLikert,
        preamble: '<h2>And finally</h2>',
        questions: [{
            prompt: 'How confident are you that the order of events you reported is correct?',
