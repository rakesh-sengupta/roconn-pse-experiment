 ...
 *      Determined from URL query string or random fallback.
 *      Example:  ?cell=C1&sg=SG2&pid=P0123
 * ----------------------------------------------------------------------- */
ROCONN.assignCondition = function () {
    const url   = new URLSearchParams(window.location.search);
    const cell  = url.get('cell') || pickRandom(Object.keys(ROCONN.cells));
    const sg    = url.get('sg')   || pickRandom(Object.keys(ROCONN.subgroups));
    const pid   = url.get('pid')  || 'P' + Date.now().toString().slice(-6);
